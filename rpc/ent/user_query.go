...
// QueryTotp chains the current query on the "totp" edge.
func (_q *UserQuery) QueryTotp() *UserTotpQuery {
	query := (&UserTotpClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithTotp tells the query-builder to eager-load the nodes that are connected to
// the "totp" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithTotp(opts ...func(*UserTotpQuery)) *UserQuery {
	query := (&UserTotpClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) loadPositions(ctx context.Context, query *PositionQuery, nodes []*User, init func(*User), assign func(*User, *Position)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(position.FieldID), joinT.C(user.PositionsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.PositionsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.PositionsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (_q *UserQuery) loadTotp(ctx context.Context, query *UserTotpQuery, nodes []*User, init func(*User), assign func(*User, *UserTotp)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
