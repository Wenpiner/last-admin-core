...
func (_u *APIUpdate) check() error {
	...
	if v, ok := _u.mutation.ServiceName(); ok {
		if err := api.ServiceNameValidator(v); err != nil {
			return &ValidationError{Name: "service_name", err: fmt.Errorf(`ent: validator failed for field "API.service_name": %w`, err)}
		}
	}
	if v, ok := _u.mutation.APIGroup(); ok {
		if err := api.APIGroupValidator(v); err != nil {
			return &ValidationError{Name: "api_group", err: fmt.Errorf(`ent: validator failed for field "API.api_group": %w`, err)}
		}
	}
	return nil
}
